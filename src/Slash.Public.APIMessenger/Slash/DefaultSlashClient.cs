...
{
    public virtual string KeysEndpoint => "keys";
    public virtual string MessageEndpoint => "message";

    /// <summary>
    /// Name of the HttpClient used for basic requests requests (Unauthenticated requests)
    /// </summary>
    public const string BasicClientName = "SlashBasicClient";

    /// <summary>
    /// Name of the HttpClient used for DPoP requests.
    /// </summary>
    public const string DPoPClientName = "SlashDPoPClient";
    ...
    public DefaultSlashClient(ILogger<DefaultSlashClient> logger, IHttpClientFactory httpClientFactory)
    {
        _logger = logger;
    ...
    public virtual async Task<List<PublicKeyInfo>> GetPublicKeys()
    {
        ...
        try
        {
            ...
            var response = await httpClient.GetAsync(KeysEndpoint);
    ...
    public virtual async Task<SendMessageResponse> SendMessage(SlashMessage message)
    {
        ...
        var httpClient = _httpClientFactory.CreateClient(DPoPClientName);
        ...
        try
        {
            ...
            response = await httpClient.PostAsync(MessageEndpoint, content);
