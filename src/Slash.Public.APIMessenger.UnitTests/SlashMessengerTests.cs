...
namespace Slash.Public.SlashMessenger.UnitTests;
...
public class SlashMessengerTests
{
    ...
    public async Task HelseIdClientShouldCacheAccessToken()
    {
        ...
        slashHttpFactoryMock.Setup(x => x.CreateClient(DefaultSlashClient.BasicClientName)).Returns(()=>CreateSlashDefaultClient());
        slashHttpFactoryMock.Setup(x => x.CreateClient(DefaultSlashClient.DPoPClientName)).Returns(()=> CreateSlashDPoPClient());

        var helseIdHttpFactoryMock = new Mock<IHttpClientFactory>();
        helseIdHttpFactoryMock.Setup(x => x.CreateClient(DefaultHelseIdClient.BasicClientName)).Returns(()=>CreateHelseIdClient());
        ...
        var slashClient = new DefaultSlashClient(new NullLogger<DefaultSlashClient>(), slashHttpFactoryMock.Object);
